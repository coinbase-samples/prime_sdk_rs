 ...
 */
// #docs operationId: PrimeRESTAPI_GetMarginSummaries
// #docs operationName: List Margin Call Summaries
use prime_sdk_rs::{
    client::PrimeClient,
    services::financing::{FinancingService, ListMarginSummariesRequest},
};
use std::env;
use std::process;
...
/**
 * Usage:
 * ./run_example.sh financing/list_margin_summaries [START_DATE] [END_DATE]
 *
 * Requires ENTITY_ID. Dates should be in YYYY-MM-DD format. Defaults: 2025-01-01T00:00:00Z to today at 00:00:00Z.
 */
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut request = ListMarginSummariesRequest::new(&entity_id);
    ...
    println!(
        "📋 Submitting list_margin_summaries request: {:#?}",
        request
    );
    match financing_service.list_margin_summaries(request).await {
        Ok(response) => {
            println!("✅ Margin summaries response: {:#?}", response);
        }
        Err(e) => {
            eprintln!("❌ Failed to list margin summaries: {}", e);
        }
    }
    Ok(())
}
