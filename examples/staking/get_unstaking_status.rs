...
use prime_sdk_rs::services::staking::{GetUnstakingStatusRequest, StakingService};
use prime_sdk_rs::PrimeClient;
use std::env;

/**
 * Usage:
 * ./run_example.sh staking/get_unstaking_status
 */
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = GetUnstakingStatusRequest::new(&portfolio_id, &wallet_id);

    println!("📋 Submitting get_unstaking_status: {:#?}", request);
    match service.get_unstaking_status(request).await {
