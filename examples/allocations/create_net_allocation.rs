...
use prime_sdk_rs::services::allocations::{AllocationService, CreateNetAllocationRequest};
...
/**
 * Usage:
 * ./run_example.sh allocations/create_net_allocation
 *
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let netting_id = Uuid::new_v4().to_string();
    ...
    let mut request = CreateNetAllocationRequest::new();
    ...
    request.netting_id = Some(netting_id);

    println!("📋 Submitting create_net_allocation request: {:#?}", request);
    match service.create_net_allocation(request).await {
        Ok(response) => println!("✅ Net allocation created! Response: {:#?}", response),
        Err(e) => eprintln!("❌ Failed to create net allocation: {}", e),
    }
